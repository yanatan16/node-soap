...
Client.prototype._invoke = function(method, arguments, location, callback) {
    ...
    xml = "<soap:Envelope " +
            ...
            "<soap:Header>" +
                ...
                (self.security && self.security.toXML ? self.security.toXML() : "") +
            "</soap:Header>" +
            "<soap:Body>" +
                message +
            "</soap:Body>" +
    ...
    http.request(location, xml, function(err, response, body) {
        if (err) {
            ...
            return callback(new Error('Empty body response!'));
        }
        else {
            try {
            ...
            if (self.security && self.security.parseHeaders) {
                self.security.parseHeaders(response.headers);
            }
